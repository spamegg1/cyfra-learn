...
class HorizBlurSuite extends ImgSuite:
  test("HorizBlur.blur with radius 1 should correctly blur the entire 3x3 image"):
    val w          = 3
    val h          = 3
    val src        = Img(w, h)
    ...
    HorizBlur.blur(src, dst, 0, 2, 1)

    check(0, 0, 2); check(1, 0, 2); check(2, 0, 3)
    check(0, 1, 3); check(1, 1, 4); check(2, 1, 4)
    check(0, 2, 0); check(1, 2, 0); check(2, 2, 0)

  test("HorizBlur.parBlur with radius 2 should correctly blur the entire 4x3 image"):
    val w          = 4
    val h          = 3
    val src        = Img(w, h)
    given dst: Img = Img(w, h)
